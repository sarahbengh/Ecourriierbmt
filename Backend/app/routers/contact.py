...
def add_contact():
    ...
    if Contact.query.filter_by(email=email, utilisateur_id=utilisateur_id).first():
        return jsonify({'error': 'Ce contact existe déjà pour cet utilisateur.'}), 400
    ...
    return jsonify({'message': 'Contact ajouté avec succès !'})